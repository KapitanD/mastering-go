...
func main() {
	logs := []string{"127.0.0.1 - - [16/Nov/2017:10:49:46 +0200] 325504",
		...
		"[17.01.2021 00:23:21 +0300]",
	}

	for _, logEntry := range logs {
		r1 := regexp.MustCompile(`.*\[(\d\d\/\w+/\d\d\d\d:\d\d:\d\d:\d\d.*)\].*`)
		r2 := regexp.MustCompile(`.*\[(\d\d\.\d\d\.\d\d\d\d\s\d\d:\d\d:\d\d.*)\].*`)
		if r1.MatchString(logEntry) {
			match := r1.FindStringSubmatch(logEntry)
		...
		} else if r2.MatchString(logEntry) {
			match := r2.FindStringSubmatch(logEntry)
			dt, err := time.Parse("02.01.2006 15:04:05 -0700", match[1])
			if err == nil {
				newFormat := dt.Format(time.RFC850)
				fmt.Println(newFormat)
			} else {
